'use strict';
// jsxl-filters compiled with jsxl-filters on Mon Apr 10 2023 22:10:52 GMT+0530 (India Standard Time)
...
Object.assign(filter, { "Type1{}":
	function(context, functions, renamed, next) { // v1, key: Type1{}, timestamp: 1681144852249
		...
		async.series([ // any all
			next => (next => { // any option 
				async.waterfall([
					next => {
						...
						if (!utils.isTypeName(source[key], 'Array')) return utils.mustBeType(context, source, key, 'Array', next);
			...
			next => (next => { // any option 
				async.waterfall([
					next => {
						if (source[key] === undefined) return utils.mustBeProvided(context, next);
						if (!utils.isTypeName(source[key], 'Object')) return utils.mustBeType(context, source, key, 'Object', next);
						target[context.rename] = source[key];
						next(null);
					}
				], utils.nullifyTrue(next));
			...
			next => (next => { // any option 
				async.waterfall([
					next => {
						if (source[key] === undefined) return utils.mustBeProvided(context, next);
						if (!utils.isTypeName(source[key], 'String')) return utils.mustBeType(context, source, key, 'String', next);
						target[context.rename] = source[key];
						next(null);
					}
				], utils.nullifyTrue(next));
				...
				async.series([ // functions
					next => { // entry
						async.waterfall([
							next => {
								if (source[key] === undefined) return utils.mustBeProvided(context, next);
								if (!utils.isTypeName(source[key], 'Function')) return utils.mustBeType(context, source, key, 'Function', next);
								target[context.rename] = source[key];
								next(null);
							}
						], utils.nullifyTrue(next));
...
filter["Type1{}"].timestamp = 1681144852249;
filter["Type1{}"].compiler  = "v1";

Object.assign(filter, { "Shell1{}":
	function(context, functions, renamed, next) { // v1, key: Shell1{}, timestamp: 1681144852229
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
				utils.buildObject(context, (keys, renamed, next) => {
					async.series([ // entries
						next => utils.filterKeys(context, [ "$id" ], keys, (context, source, target, key, next) => { next = utils.defer(utils.nullifyTrue(next)); // object entries
							async.waterfall([
								next => {
									if (source[key] === undefined) return next(true); // optional
									if (!utils.isTypeName(source[key], 'String')) return utils.mustBeType(context, source, key, 'String', next);
									target[context.rename] = source[key];
									next(null);
								}
						...
						next => utils.filterKeys(context, [ "$remove" ], keys, (context, source, target, key, next) => { next = utils.defer(utils.nullifyTrue(next)); // object entries
							async.waterfall([
								next => {
									if (source[key] === undefined) return next(true); // optional
									if (!utils.isTypeName(source[key], 'Boolean')) return utils.mustBeType(context, source, key, 'Boolean', next);
									target[context.rename] = source[key];
									next(null);
								}
							], utils.nullifyTrue(next));
							...
							async.series([ // any all
								...
								next => (next => { // any option 
									async.waterfall([
										next => {
											if (source[key] === undefined) return next(true); // optional
											if (!utils.isTypeName(source[key], 'Number')) return utils.mustBeType(context, source, key, 'Number', next);
											target[context.rename] = source[key];
											next(null);
										}
									], utils.nullifyTrue(next));
						...
						next => utils.filterKeys(context, [ "$scope" ], keys, (context, source, target, key, next) => { next = utils.defer(utils.nullifyTrue(next)); // object entries
							async.series([ // any all
								next => (next => { // any option 
									async.waterfall([
										next => {
...
filter["Shell1{}"].timestamp = 1681144852229;
filter["Shell1{}"].compiler  = "v1";

Object.assign(filter, { "Type1{$optional:true}":
	function(context, functions, renamed, next) { // v1, key: Type1{$optional:true}, timestamp: 1681144852242
		const source = context.source, target = context.target, key = context.key;
		async.series([ // any all
			next => (next => { // any option 
				async.waterfall([
...
filter["Type1{$optional:true}"].timestamp = 1681144852242;
filter["Type1{$optional:true}"].compiler  = "v1";

Object.assign(filter, { "Object1{}":
	function(context, functions, renamed, next) { // v1, key: Object1{}, timestamp: 1681144852290
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter["Object1{}"].timestamp = 1681144852290;
filter["Object1{}"].compiler  = "v1";

Object.assign(filter, { "Array1{}":
	function(context, functions, renamed, next) { // v1, key: Array1{}, timestamp: 1681144852291
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
...
filter["Array1{}"].timestamp = 1681144852291;
filter["Array1{}"].compiler  = "v1";

Object.assign(filter, { "type1":
	function(context, functions, renamed, next) { // v1, key: type1, timestamp: 1681144852219
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter["Type1{}"](context, utils.intrinsic.filter["Type1{}"].functions, renamed, next);
	}
});
filter["type1"].functions = [];
filter["type1"].timestamp = 1681144852219;
filter["type1"].compiler  = "v1";

Object.assign(filter, { "shell1":
	function(context, functions, renamed, next) { // v1, key: shell1, timestamp: 1681144852228
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter["Shell1{}"](context, utils.intrinsic.filter["Shell1{}"].functions, renamed, next);
	}
});
filter["shell1"].functions = [];
filter["shell1"].timestamp = 1681144852228;
filter["shell1"].compiler  = "v1";

Object.assign(filter, { "object1":
	function(context, functions, renamed, next) { // v1, key: object1, timestamp: 1681144852289
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter["Object1{}"](context, utils.intrinsic.filter["Object1{}"].functions, renamed, next);
	}
});
filter["object1"].functions = [];
filter["object1"].timestamp = 1681144852289;
filter["object1"].compiler  = "v1";
...
Object.assign(filter, { "Type2{}":
	function(context, functions, renamed, next) { // v2, key: Type2{}, timestamp: 1681144852562
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
					...
					next => (next => {
						async.series([ // functions
							...
							next => { // entry
								async.waterfall([
									next => {
										context.scope = Object.assign(context.scope, {
												inside: "object"
										});
										if (source[key] === undefined) return utils.mustBeProvided(context, next);
										utils.buildObject(context, (keys, renamed, next) => {
...
filter["Type2{}"].functions = [
		(context, filter, next) => next(null, utils.isType(filter, String)),(context, filter, next) => compiler2.compileString(context, next),(context, filter, next) => {
				...
				},(context, filter, next) => compiler2.compileShell(context, next),(context, filter, next) => next(null, utils.isFunction(filter)),(context, filter, next) => compiler2.compileFunction(context, next),(context, filter, next) => next(null, utils.isObject(filter)),(context, filter, next) => compiler2.compileObject(context, next),(context, filter, next) => { 
...
filter["Type2{}"].timestamp = 1681144852562;
filter["Type2{}"].compiler  = "v2";

Object.assign(filter, { "Shell2{}":
	function(context, functions, renamed, next) { // v2, key: Shell2{}, timestamp: 1681144852580
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter["Shell2{}"].timestamp = 1681144852580;
filter["Shell2{}"].compiler  = "v2";

Object.assign(filter, { "Type2{$optional:true}":
	function(context, functions, renamed, next) { // v2, key: Type2{$optional:true}, timestamp: 1681144852584
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
				utils.callFilter(context, functions[13], (err, keep) => { if (err) return next(err);
...
filter["Type2{$optional:true}"].timestamp = 1681144852584;
filter["Type2{$optional:true}"].compiler  = "v2";

Object.assign(filter, { "Shell2{$optional:true}":
	function(context, functions, renamed, next) { // v2, key: Shell2{$optional:true}, timestamp: 1681144852596
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter["Shell2{$optional:true}"].timestamp = 1681144852596;
filter["Shell2{$optional:true}"].compiler  = "v2";

Object.assign(filter, { "type2":
	function(context, functions, renamed, next) { // v2, key: type2, timestamp: 1681144852559
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter["Type2{}"](context, utils.intrinsic.filter["Type2{}"].functions, renamed, next);
	}
});
filter["type2"].functions = [];
filter["type2"].timestamp = 1681144852559;
filter["type2"].compiler  = "v2";

module.exports.timestamp = 1681144852691;
