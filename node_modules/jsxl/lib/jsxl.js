...
if (testing) {
	...
	trackState = 2;	// must be 0-4 and >= startState
	stopState  = 2;	// must be 1-4 and >= trackState
	verbose = true;
} else {
...
function normalizeError(func) {
	return (...args) => {
		const context = args[0];
		if (context && context.constructor === Context) return func(...args);
		const next = args[args.length - 1];
		args[args.length - 1] = (...args) => { 
			const err = args[0];
			if (err) { 
				const message = `(${err.action || 'missing action'}) ${utils.errorPrefix(err)}`;
				const n = message.split('\n').length;
				err.stack = err.stack.split('\n');
				err.stack.splice(0, n, `Error: ${message}`)
...
module.exports = whenInitiated(normalizeError(v2 ? compiler2.inline : compiler1.inline));
module.exports.compile = whenInitiated(normalizeError(v2 ? compiler2.compile : compiler1.compile));
module.exports.compileMultiple = whenInitiated(normalizeError(v2 ? compiler2.compileMultiple : compiler1.compileMultiple));	
module.exports.execute = whenInitiated(normalizeError(utils.execute));
...
function functional(types) {
	return [ Function ].concat(types.constructor === Array ? types : [ types ]);
}
...
// TODO how do we type check Functional return values ?		
		'$key+$type':					{ $optional: true, $type: type }, 								// require listed type as value and as key
		'$any+$switch':					{ $optional: true, $type: [ type ] }, 							// require any of listed types
		
		$remove:						{ $optional: true, $any: functional(globals.booleanType) },		// remove item from target
		...
		$map:							{ $optional: true, $any: functional(globals.collectionTypes)}, 	// translate from key to entry,  if source is undefined, if function, then to be executed at runtime time with parameters
		$optional:						{ $optional: true, $any: functional(globals.booleanType) }, 	// accept if source is undefined

		$parallel:						{ $optional: true, $type: Boolean }, 							// run array or object filtering in parallel as opposed to in series

		$message:						{ $optional: true, $any: functional(globals.messageType) }, 	// custom message for any failed assertion
		'$lt+$lte+$eq+$ne+$gte+$gt':	{ $optional: true, $any: functional(globals.scalarTypes) },		// assertions, item is compared, if function, then to be executed at runtime time with parameters
		'$in+$nin':						{ $optional: true, $any: functional(globals.collectionTypes) }, // assertions, all items are related in a lean way, possibly matching if RegExp is provided, if function, then to be executed at runtime time with parameters
		'$inc+$ninc':					{ $optional: true, $any: functional(globals.scalarTypes) },	 	// assertions, all items are related in a lean way, possibly matching if RegExp is provided, if function, then to be executed at runtime time with parameters // $any: [ Function, Boolean, Number, String, RegExp, Date ] 
		$match:							{ $optional: true, $any: functional(globals.scalarTypes) },	 	// assertion, item is matched, if function, then to be executed at runtime time with parameters // $any: [ Function, any Scalar ] 
		'$name+$constructor':			{ $optional: true, $any: functional(globals.nameType) }, 	// assertion, item name constructor name is compared, if function, then to be executed at runtime time with parameters
		'$length+$minlen+$maxlen':		{ $optional: true, $any: functional(globals.lengthType) },	// assertions, item length is compared, if function, then to be executed at runtime time with parameters

		$rename: 						{ $optional: true, $any: functional(globals.nameType) }, 	// rename target item
										
		'$toArray+$toObject':			{ $optional: true, $any: functional(globals.nameType) }, 		// switch between array and object, use modifier value as key
		'$filter+$transform+$final':	{ $optional: true, $type: Function }, 							// filter next(err, keep), transform next(err, value), and final next(err) 
		
		$scope:							{ $optional: true, $any: functional(globals.scopeType) },	// allow values to be carried down
	...
	Boolean: {  $any: [ 
		...
		{ 
			...
			$transform: (context, string, next) => {
				...
				next(new Error('is an invalid Boolean string'));
			}
		}
	]},
	Number: {  $any: [ 
		...
		{ 
			...
			$transform: (context, string, next) => {
				...
				if (isNaN(string)) return next(new Error('is an invalid Number string'));
				next(null, string);
			}
		}
	]},
	IntNumber: {  $any: [ 
		...
		{ 
			...
			$transform: (context, string, next) => {
				...
				if (isNaN(string)) return next(new Error('is an invalid international Number string'));
				next(null, string);
			}
		}
	]},
	Date: {  $any: [ 
		...
		{ 
			...
			$transform: (context, string, next) => {
				...
				if (string == 'Invalid Date') return next(new Error('is an invalid Date string'));
				next(null, string);
			}
		}
	]},
	RegExp: {  $any: [ 
		...
		{ 
			...
			$transform: (context, string, next) => {
				...
				next(new Error('is an invalid RegExp string'));
			}
		}
	]},
};
...
// or ignore because this only happens when recompiling good filters stored to intrinsic.js	
	...
	next => { // 0 - 1
		...
		compiler1.compileMultiple(filters1, (err, filters) => {
