...
//
const compile = 
module.exports.compile = (filter, options, next) => compiler3.compile(filter, options, compileType, 'v2', next);
...
const compileType = module.exports.compileType = (context, next) => { // next(err, script)
	async.waterfall([ 
		next => {
if (context.source[context.key] === undefined) console.log(3333, context)	// && context.source[context.key] !== undefined 
			if (context.source[context.key] !== null && context.source[context.key].constructor === Script) return next(null, context.source[context.key]); // avoid recompiling
...
const compileShell = module.exports.compileShell = (context, next) => { // next(err, script)
	...
	async.waterfall([
		...
		(script, next) => {
			if (filter.$key) 
				script.links.key.substitute(new Script(...coder.sourceKey(filter.$key)));
