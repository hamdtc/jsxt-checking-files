...
const compileType = module.exports.compileType = (context, next) => { // next(err, script)
	async.waterfall([ 
		next => {
			if (context.key === '$type' && (! ('$type' in context.source))) return next(null, null); 
		...
		(filter, next) => {
			...
			case Object:   
				...
				if ('$escape' in filter) {
					...
					if (Object.keys(filter).length > 1) return next(utils.ctx(context, new Error(`.$escape cannot mix with other attributes`)));
					return utils.execute(context, utils.intrinsic.filter.object1 || null, (err, filter) => { if (err) return next(err);
						context.target = {}; // clear previous execution to avoid shifting when source === target
						context.scope.inside = 'object';
			...
			default:
				return next(utils.ctx(context, new Error(`holds illegal filter type: ${filter}`)));
			}
		}
	], next);
}
...
// TODO does this also apply to compileSwitch() ?
			// create a working copy of modifiers, this allows inheritance but avoids parent and sibling override
	...
	async.waterfall([
		next => {
			if (context.scope.inside == 'array') {
				if (filter.$key) return next(new Error(`.$key cannot filter key when inside array`))
				if (filter.$rename) return next(new Error(`.$rename cannot rename key when inside array`))
			}

			// disallow mix of type modifiers
			if (('$type' in filter ? 1 : 0) + ('$any' in filter ? 1 : 0) + ('$switch' in filter ? 1 : 0) > 1) return next(utils.ctx(context, new Error(`holds multiple of $type, $any, or $switch modifiers`)));  
			...
			async.forEachOfSeries(filter, (entry, key, next) => { next = utils.defer(next);
				// no need to ignore possible undefined entries, filter is target side
				
				if (context.scope.insideKey && !globals.allowedInKey.includes(key)) return next(utils.ctx(context, new Error(`makes illegal use of ${key} in context of $key`)));

				// disallow mix of modifiers and other keys
				if (! (utils.modifiers.includes(key))) {
					return next(utils.ctx(context, new Error(`.${key} mixes with $-modifiers`))); 
				}
				...
				if (!(key in modifiers)) modifiers[key] = entry;
				next();
			}, next);
		},
		// compile key modifier, if any
		next => {
			if (filter.$key) {
				// create a working copy of modifiers, this allows inheritance but avoids parent override
				const modifiers = Object.assign({}, context.scope.modifiers);
				let ctx = new Context(context, '$key').setScopeEntry('inside', undefined).setScopeEntry('insideKey', true).setScopeEntry('modifiers', {});

//				delete ctx.scope.inside; 
//				ctx.scope.insideKey = true;
// 				ctx.scope.modifiers = {};
				return compileType(ctx, (err, key) => {
					context.scope.modifiers = Object.assign({}, modifiers); // reset modifiers
					next(err, key)
				}); 
			}
		...
		(key, next) => {
			...
			})((err, script) => next(err, key, script));
		},
		// compile surrounding functions
		(key, script, next) => {
			if (key) 
				script.links.key.substitute(new Script(...coder.sourceKey(key)));
