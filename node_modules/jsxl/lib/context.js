...
const Context = module.exports = function Context(context, key, rename) {
	// TODO remove	
	if (context.key === key && key == 'Object{}') throw new Error('illegal/suspicious context construction: ' + key)
...
Context.prototype.fullpath = function() {
	...
	case String: return this.path + (this.key.match(/^[\$a-zA-Z_][\.\:\$a-zA-Z_0-9]*$/) ? (this.path ? '.' : '') + `${this.key}` : this.key.match(/^[0-9]+$/) ? `[${this.key}]` : `[${JSON.stringify(this.key)}]`);
